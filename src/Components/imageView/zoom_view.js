...
const positionMult = (input, value) => {
  return {x: input.x * value, y: input.y * value};
};
...
export default class ZoomView extends Component {
  constructor(props) {
    ...
    this.gestureHandlers = {
      ...
      onResponderRelease: _event => {
        ...
        if (
          this.scaleTarget > MIN_ZOOM_RESET &&
          this.scaleTarget < MAX_ZOOM_RESET
        ) {
    ...
    switch (event.nativeEvent.touches.length) {
      ...
      case 2: {
        ...
        const newPinch = {
          ...
          distance: Math.sqrt(
            Math.pow(touchA.x - touchB.x, 2) + Math.pow(touchA.y - touchB.y, 2),
          ),
        };
        if (this.childState.status === ChildStatus.DOUBLE) {
          this.doChildPinchMove(
            this.childState.last,
            newPinch,
  ...
  doChildAnimation(timing, callback = () => {}, override = false) {
    if (this.doingChildAnimation === true && !override) return;
    if (override) {
    ...
    this.childDoingZoom = true;
    const boundScaleTarget = Math.max(
      Math.min(this.scaleTarget, MAX_ZOOM),
      MIN_ZOOM,
    );
    ...
    Animated.parallel(
      [
        ...
        Animated.timing(this.transformAnimated, {
          duration: timing,
  ...
  getCenterDisposition(pos) {
    ...
    if (pos.x > 0) return {x: pos.x - center.x, y: pos.y - center.y};
    else return {x: center.y + 50 + pos.x - center.x, y: pos.y - center.y};
  }
  ...
  doChildPinchMove(prev, next, timeDiff) {
    ...
    this.doChangeScale(
      newDisposition,
      updateScale,
      this.getCenterDisposition(prev.center),
    );
  ...
  doChangeScale(updateDisposition, updateScale, updateCenter) {
    ...
    let newCenterDisposition = positionMult(
      centerToCenterDisposition,
      1 - updateScale,
    );
    const disposition = {
